...
namespace test111
{
 ...
 //               Console.WriteLine("csrfToken:" + csrfToken);
...

            {
                ...
                if (!_match.Success)
                {
                    throw new Exception("无法获取StepId,可能是因为不在打卡时间内");
                    return false;
                }
        ...
        static async System.Threading.Tasks.Task<bool> Card(User user)
        {
            ...
            if (res)
            {
                test111.Form1.flg1 = 1;
                await Commit(user, client);
            }
            else throw new Exception("打卡失败" + user.username);
            return res;
        }
        ...
        public static async System.Threading.Tasks.Task daka( )
        {
            ...
            int op = 2;
            {
                //
                User user = new User();
                var path = @"E:\C#\dakatest\test111\bin\Debug\user.json";
 ...
 //           Console.WriteLine("quit");
        ...
        private static async System.Threading.Tasks.Task<string> getbjAsync(HttpClient client, String nj, String bj, String csrfToken, String entryId)
        {
            ...
            var baseUrl = "https://ehall.jlu.edu.cn/infoplus/interface/suggest+30";
